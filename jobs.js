  ...
  users.forEach(async (user) => {
    ...
    if (events.length > 0) {
      ...
      if (eventsToSend.length > 0) {
        ...
        message += `Hello,<br />`;
        message += `You have ${eventsToSend.length} upcoming events:<br />`;
        message += `<table border='1' style='border-collapse:collapse'>`;
        message += `<thead>`;
        message += `<tr>`;
        message += `<th>Event Name</th>`;
        message += `<th>Date Time</th>`;
        message += `<th>ETA</th>`;
        eventsToSend.forEach((event) => {
          ...
          message += `<td style='padding:10px;text-align:center;'>${ndt.toUTCString().slice(0, -4)}</td>`;
...
const scheduler = () => {
  ...
  cron.schedule("*/10 * * * * *", async () => {
    ...
    try {
      ...
      console.log("Error sending updates : ", error);
    }
  });
};
